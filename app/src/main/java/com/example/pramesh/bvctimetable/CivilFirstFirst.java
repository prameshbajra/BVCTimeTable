...
public class CivilFirstFirst extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_civil_first_first);

        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setDisplayShowHomeEnabled(true);

        String sectiondata = getIntent().getStringExtra("sectiondata");
        Toast.makeText(this, "Please wait! It might take upto 20 seconds to load for the first time", Toast.LENGTH_LONG).show();

        if (sectiondata.equals("sectionA")) {
            firebaseData = FirebaseDatabase.getInstance().getReference().child("civiltableoneoneA");
        } else if (sectiondata.equals("sectionB")) {
            firebaseData = FirebaseDatabase.getInstance().getReference().child("civiltableoneoneB");
        } else if (sectiondata.equals("sectionC")) {
            firebaseData = FirebaseDatabase.getInstance().getReference().child("civiltableoneoneC");
        }
    ...
    public void alertDial(String value) {
        AlertDialog.Builder builder = new AlertDialog.Builder(CivilFirstFirst.this);
